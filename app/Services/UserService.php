...
class UserService
{
        ...
        try {
            ...
            if ($userReset) {
                ...
                if ($user) {
                    ...
                    PasswordResetRepository::deleteUser($userReset, $isUser);
     ...
     * @return array
     */
    public function allUser($input)
    {
        return $this->userRepository->getAllUser($input)->orderBy('id', 'DESC')->paginate($input->per_page);
    }
    ...
    {
        try {
            $newStatus = $this->userRepository->changeStatus($id_user);
            if ($newStatus == 0 && $id_user == Auth::guard('user')->user()->id) {
                // Session::flush();
                Auth::guard('user')->logout();
            }
        } catch (\Exception $e) {
     ...
     * @return object
     */
    public function ajaxSearchUserAdmin($input)
    {
        try {
            $users = $this->userRepository->ajaxSearchUser($input)->orderBy('id', 'DESC')->paginate($input->per_page);
            $renderHtml = view('admin.render_ajax.user', compact('users'))->render();
